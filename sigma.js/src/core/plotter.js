...
function Plotter(nodesCtx, edgesCtx, labelsCtx, hoverCtx, graph, w, h) {
  ...
  function drawEdge(edge) {
    ...
    // Compute displayX and displayY for drawLabel of the current edge before applyArrow
    edge.displayX = (sourceCoordinates[0] + targetCoordinates[0]) / 2;
    edge.displayY = (sourceCoordinates[1] + targetCoordinates[1]) / 2;
    ...
    switch (edge['type'] || self.p.defaultEdgeType) {
      case 'curve':
        ...
        // Re-adjust with controlPoint
        edge.displayX = (edge.displayX + controlPointX) / 2;
        edge.displayY = (edge.displayY + controlPointY) / 2;
    ...
    edge.displaySize = edge.source.displaySize;
    // Draw the edge's label
    drawLabel(edge);
