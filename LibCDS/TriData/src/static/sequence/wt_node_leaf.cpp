...
namespace cds_static
{
	...
	size_t wt_node_leaf::rank_prefix(uint *symbol, size_t pos, uint l, wt_coder *c, uint deep) const
	{
		return pos+1;
	}
    ...
    {
    	//	cout << "symb" <<  *symbol << " " << this->symbol << endl;
    ...
    size_t wt_node_leaf::select_prefix(uint *symbol, size_t pos, uint l, wt_coder *c, uint deep) const
        {
	      //if(symbol!=this->symbol) return (size_t)-1;
	              if(pos==0 || pos>count) return (size_t)-1;
	                      return pos;
