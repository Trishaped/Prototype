...
namespace cds_static
{
   ...
   //     cout << "level " << l << endl;
        ...
        delete [] ibitmap;
        size_t count_right = bitmap->rank1(n-1); // nb of 1's
        size_t count_left = n-count_right+1; // nb of 0's
        ...
        for(uint i=0;i<n;i++) {
       // 	cout << bitmap->access(i) << " ";
            if(bitmap->access(i)) {
                right[count_right++]=symbols[i];
                if(count_right>1)
                    if(right[count_right-1]!=right[count_right-2]) //at least 2 different symbols for the right node
                        match_right = false;
            }
            else {
                left[count_left++]=symbols[i];
                if(count_left>1)
                    if(left[count_left-1]!=left[count_left-2]) //at least 2 different symbols for the left node
                        match_left = false;
            }
        }
    /*    cout << endl;
...
*/
		return rank_prefix(symbol, pos, l, c, c->depth());
    }

    size_t wt_node_internal::rank_prefix(uint * symbol, size_t pos, uint l, wt_coder *c, uint deep) const
    {
        bool is_set = ((*symbol& (1 << (deep-1)))!=0);
//        cout << "pos:" << pos << " deep:" << deep << " is_set:" << is_set << endl;
        if(!is_set) {
        	if(deep==1){
        		return bitmap->rank0(pos);
        	}else{
        		if(left_child==NULL)
        			return 0;
            	return left_child->rank_prefix(symbol, bitmap->rank0(pos)-1,l+1, c, deep-1);
            }
        }else{
        	if(deep==1){
        	    ...
        	    if(right_child==NULL)
        			return 0;
            	return right_child->rank_prefix(symbol, bitmap->rank1(pos)-1,l+1, c, deep-1);
            }
        }
    }
...
/*      bool is_set = c->is_set(symbol,l); //c->is_set(symbol, l);
        ...
        return ret;
    */
    	return select_prefix(symbol, pos, l, c, c->depth());
    }

    size_t wt_node_internal::select_prefix(uint * symbol, size_t pos, uint l, wt_coder *c, uint deep) const
    {
        bool is_set = ((*symbol& (1 << (deep-1)))!=0);
   //     cout << "pos:" << pos << " deep:" << deep << " is_set:" << is_set << endl;
		size_t ret = 0;
        if(!is_set) {
	    	if(deep==1){
	    		ret = bitmap->select0(pos)+1;
	    	}else{		
	        	if(left_child==NULL)
	            	return (size_t)(-1);
	            size_t new_pos = left_child->select_prefix(symbol, pos, l+1, c,deep-1);
		    ...
		    if(deep==1){
		        ret = bitmap->select1(pos)+1;
	    	}else{
	        	if(right_child==NULL)
	            	return (size_t)(-1);
	            size_t new_pos = right_child->select_prefix(symbol, pos, l+1, c,deep-1);
    	        if(new_pos+1==0) return (size_t)(-1);
        	    ret = bitmap->select1(new_pos)+1;
	    	}
        }
