...
namespace cds_static
{
	...
	size_t Sequence::rank_prefix(uint c, size_t i, uint deep) const{
		return rank(c,i);
	}
	...
	size_t Sequence::select_prefix(uint c, size_t j, uint deep) const{
		return select(c,j);
	}
	...
	Sequence * Sequence::load(ifstream & fp) {
		...
		switch(type) {
			...
			case ADJLIST_HDR: return AdjacentListWithRankAndSelect::load(fp);
		}
		return NULL;
	}
