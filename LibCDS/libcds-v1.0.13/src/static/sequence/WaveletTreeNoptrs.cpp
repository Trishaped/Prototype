...
namespace cds_static
{
	...
	size_t WaveletTreeNoptrs::rank(uint symbol, size_t pos) const
	{
		...
		return count;*/
		return rank_prefix(symbol,pos,height);
	}

	size_t WaveletTreeNoptrs::rank_prefix(uint symbol, size_t pos, uint deep) const
	{
		//symbol = am->map(symbol);;
		size_t start = 0;
		size_t count = 0;
	cout << "rank_prefix" << endl;	
	for(uint level = height-deep; level < height; level++) {
			uint masked = (symbol >> (height - level - 1)) << (height - level - 1);
...
/*		symbol = am->map(symbol);
		...
		return pos - 1;*/
		return select_prefix(symbol, j, height);
	}
	
	size_t WaveletTreeNoptrs::select_prefix(uint symbol, size_t j, uint deep) const{
		//symbol = am->map(symbol);
