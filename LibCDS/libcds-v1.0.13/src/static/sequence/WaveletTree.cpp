...
namespace cds_static
{
    ...
    {
        size_t ret = root->rank(&symbol, pos, 0, c);
        return ret;
    }

    size_t WaveletTree::rank_prefix(uint symbol, size_t pos, uint deep) const
    {
        //uint * s = c->get_symbol(am->map(symbol));
        //cout << (*s) << endl;
        size_t ret = root->rank_prefix(&symbol, pos, 0, c, deep);
        return ret;
    }
    ...
    {
        uint ret = root->select(&symbol, pos, 0, c);
        if(ret==((uint)-1)) {return (uint)-1;}
        return ret-1;
    }

    size_t WaveletTree::select_prefix(uint symbol, size_t pos, uint deep) const
    {
        uint ret = root->select_prefix(&symbol, pos, 0, c, deep);
        if(ret==((uint)-1)) { return (uint)-1;}
        return ret-1;
    }
